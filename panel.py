...
# нельзя починить с телефона). Замок считает НЕУДАЧИ по адресу (Caddy подставляет свой
# X-Forwarded-For и входящему не верит — проверено ревью на боевом Caddyfile) и добавляет
# им задержку. Отказать верному паролю он не может: см. порядок действий в login().
LOGIN_FAILS_BEFORE_LOCK = 5
LOGIN_LOCK_SEC = 300
# Проверки пароля идут ПО ОДНОЙ: scrypt берёт 16 МБ, и десяток одновременных проверок
# при mem_limit 256m — это OOM того самого контейнера, до которого достаёт интернет.
_CHECK_GATE = asyncio.Semaphore(1)
# Задержка ответа на НЕВЕРНЫЙ пароль с запертого адреса. Верному паролю она не стоит
# ничего: он до этой ветки не доходит.
_LOCKED_PENALTY_SEC = 1.0
...
async def login(request: Request):
    ...
    # ПАРОЛЬ СВЕРЯЕТСЯ ПЕРВЫМ, И ЭТО НЕ ОТМЕНЯЕТ НИЧТО. Верный пароль обязан проходить
    # всегда — с любого адреса, при любом замке, посреди любого флуда. Панель у этой
    # установки единственный способ заменить умерший токен без ssh, и защита, которая
    # запирает хозяина, дороже перебора: пять запросов из одной консоли или пять
    # собственных опечаток иначе оставляют владельца снаружи на пять минут.
    #
    # Перебор ограничен не отказом, а ЦЕНОЙ попытки:
    #   • scrypt считается 60 мс и по одному за раз (_CHECK_GATE) — подбор упирается
    #     в процессор, а память остаётся в пределах одной проверки (16 МБ при
    #     mem_limit 256m). Считается в отдельном потоке: hashlib отпускает GIL,
    #     и приём вебхуков паузы не замечает;
    #   • НЕУДАЧНАЯ попытка с запертого адреса отвечает с задержкой (_LOCKED_PENALTY_SEC).
    #     Верный пароль в эту ветку не попадает никогда, поэтому владельцу она ничего
    #     не стоит, а подбору стоит секунды на попытку.
    # Чего это не закрывает: ботнет с меняющихся адресов по-прежнему занимает процессор
    # проверками. Это свойство любого ограничителя без прокси, и закрывается оно на прокси.
    async with _CHECK_GATE:
        ok = await asyncio.to_thread(check_password, str(form.get("password") or ""))

    if ok:
        # Успешный вход снимает замок: он ставился на подозрение, а подозрение снято.
    ...
    if left:
        # Штраф платит только неверный пароль. Задержка держит соединение открытым
        # и режет темп подбора; верный пароль сюда не доходит.
        await asyncio.sleep(_LOCKED_PENALTY_SEC)
        return _render(
            "login.html",
            status=429,
            error=f"Слишком много попыток. Подождите {left} с — верный пароль примем и раньше.",
        )
