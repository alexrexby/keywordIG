...
Восстановление после протухания (владелец прошёл Instagram Business Login заново) идёт
через админку — POST /ig/admin/token, функция store() ниже: запись снимает invalid_at,
диспетчер сам разбирает накопившуюся очередь. Ни правки .env, ни рестарта контейнера,
то есть авария не стоит приёма вебхуков.
Запасной путь, если админка недоступна или сменился IG_TOKEN_KEY:
    ...
    if row is not None:
        if row["invalid_at"] is not None:
            ...
            await meta.alert_owner(
                ...
                "продлённый токен).\nПростой путь: /admin/instagram, поле «Новый токен "
                "Meta» — запись поверх мёртвого значения там разрешена.\nЕсли админка "
                "недоступна, снимите строку и поднимите сервис заново:\n"
                "docker compose -f docker-compose.prod.yml exec -T postgres "
                'psql -U ig -d ig -c "delete from instagram.ig_token"'
            )
        return
...
async def probe(value: str) -> None:
    ...
    if ids and IG_USER_ID not in ids:
        await meta.alert_owner(
            "Instagram: сохранённый через админку токен принадлежит ДРУГОМУ аккаунту "
            f"(ожидали {IG_USER_ID}, у токена {', '.join(sorted(ids))}).\n"
            "Ответы уйдут не от того аккаунта — проверьте, каким входили в Instagram "
            "Business Login, и вставьте токен нужного аккаунта заново."
        )
        return
...
async def mark_invalid(reason: str) -> None:
    ...
    await meta.alert_owner(
        ...
        "Починка: пройдите Instagram Business Login и вставьте свежий токен в админке — "
        "/admin/instagram, поле «Новый токен Meta». Рестарт не нужен, очередь разберётся "
        "сама.\nЕсли админка недоступна: снять строку instagram.ig_token, положить токен "
        "в переменную IG_ACCESS_TOKEN и перезапустить instagram-service."
    )
