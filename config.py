...
#
# ПОЧИНКА ПОСЛЕ ПРОТУХАНИЯ — теперь без правки этого файла и без рестарта:
# владелец проходит Instagram Business Login и отдаёт свежий токен админке
# (POST /ig/admin/token, admin.py). Запись снимает invalid_at,
# диспетчер разбирает накопившуюся очередь сам. Ради этого админка и заведена: авария
# случается раз в 60 суток, а рестарт контейнера ради неё стоил приёма вебхуков.
#
# Запасной путь (админка недоступна, или сменили IG_TOKEN_KEY — тогда строка перестаёт
...
# Ключ админ-API правил (/ig/admin/*), его знает CRM и предъявляет на входе.
# Направление одно: web → сервис. Отдельный от INTERNAL_TOKEN намеренно и по той же
# причине, что IG_ALERT_TOKEN выше: общий ключ открывает всё внутреннее API платформы,
# включая переписку менеджеров, а этот контейнер смотрит в интернет. Обратно, из сервиса
# в web, этим токеном не ходят — там свой INTERNAL_TOKEN, и сюда он не попадает.
