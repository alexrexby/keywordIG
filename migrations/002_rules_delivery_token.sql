...
-- ДОБАВЛЯЕШЬ КОЛОНКУ В ig_rule — сначала прочти db.admin_copy_rule. Копирование правила
-- (POST /ig/admin/rules/{id}/copy) размножает строку ЦЕЛИКОМ, через to_jsonb →
-- jsonb_populate_record, и список колонок берёт из самой базы: обычная колонка попадёт
-- в копию сама, править ради неё нечего — так и задумано. Опасны ровно два вида колонок,
-- и обе проверены прогоном (ревью, четвёртый круг), а не предположены:
--   1) UNIQUE (slug, внешний id) — копия ляжет с тем же значением, вторая строка не
--      пройдёт: 400 с именем ограничения. Ломается ГРОМКО, человек увидит отказ;
--   2) персональное для правила значение БЕЗ уникальности (секрет доставки, токен
--      партнёра) — скопируется дословно, и два правила молча поделят один секрет.
--      Вот это тихо, и ради этого случая предупреждение и написано.
-- Завёл такую колонку — подмени её в jsonb_build_object внутри db.admin_copy_rule,
-- рядом с id и name. Это одна строка, но сделать её должен ты, а не следующий, кто
-- будет разбираться, почему у двух правил один секрет.
