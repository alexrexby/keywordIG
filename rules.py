...
# веб-формой на этапе 5), а разбор идёт в том же event loop, где живёт приёмник вебхуков:
...
def normalize(text: str | None) -> str:
    ...
    «слово,» должны дать «слово», но «гайд!скидка» не должно склеиться в одно слово.
    """
...
def _hits(rule: Rule, text: str) -> bool:
    ...
    «гайд», «гайда», «гайдай» на одно ключевое слово, и терять их дороже, чем
...
def check_template(text: str | None) -> str | None:
    ...
    Зовётся из unconfigured для КАЖДОЙ строки правила и предназначена для формы этапа 5.
...
def unconfigured(rule: Rule) -> str | None:
    ...
    if PLACEHOLDER in rule.dm_text:
        return "в тексте DM остался плейсхолдер"
