"""Админ-API правил: CRM → сервис. CRUD, предпросмотр шаблона, замена токена, состояние.

Направление ровно одно: web стучится сюда со своим ключом IG_ADMIN_TOKEN. Обратно, из
сервиса в web, этим ключом не ходят — там свой INTERNAL_TOKEN, и в этот контейнер он не
попадает намеренно (см. config.IG_ADMIN_TOKEN и env-блок docker-compose.prod.yml).
...
# Имя заголовка ровно одно. В плане (контракт B) стоял X-Internal-Token, и алиас под этим
# именем какое-то время принимался — снят намеренно: прав он не добавлял (сверка всегда
# с IG_ADMIN_TOKEN), но назывался именем мастер-токена платформы, который из этого
# контейнера вычищен специально. Одинаковое имя рано или поздно приводит к «да это же тот
# самый ключ» — и общий INTERNAL_TOKEN оказывается в интернет-обращённом сервисе.
...
# None, а не 0.0: monotonic идёт от загрузки ХОСТА, и ноль съел бы первую сводку целиком
# (.claude/journal/cards/2026-08-26-monotonic-cooldown.md).
...
def require_admin(request: Request) -> None:
    ...
    Fail-closed по образцу signature.verify_signature и приёмнике вебхуков CRM:
    ненастроенный секрет — это «никого не пускать», а не «пускать всех». Сравнение по
    байтам постоянным временем; какой именно заголовок не подошёл, наружу не сообщаем.
    """
    if IG_ADMIN_TOKEN:
...
@router.get("/state")
async def state():
    """Состояние сервиса для баннера в форме. Секретов в ответе нет по составу полей."""
