...
async def save_rule(body: dict, current: dict | None) -> tuple[dict | None, list[str], list[str]]:
    ...
    человеку нечем.
    """
    # ВЫКЛЮЧЕНИЕ НЕ ПРОВЕРЯЕТСЯ СОДЕРЖИМЫМ. Иначе включённое правило, чьи хранимые поля
    # не проходят нынешние проверки (правку делали SQL-ом, проверки ужесточила новая
    # версия пакета, ссылка законна для Meta и незаконна для нас), выключить нельзя
    # ВООБЩЕ — ни кнопкой, ни PATCH: аварийный стоп отвечает 400 и оставляет правило
    # отвечать людям. Выключение не может сделать хуже: оно только убирает правило
    # из выдачи диспетчера. Проверки на ВКЛЮЧЕНИЕ остаются полностью.
    if current is not None and set(body) == {"enabled"} and body["enabled"] is False:
        row = await db.admin_update_rule(current["id"], {**current, "enabled": False})
        if row is not None:
            log.info("правило %s выключено: «%s»", row["id"], row["name"])
        return row, [], []
...
async def state():
    ...
    return {
        ...
        "alerts": await meta.channel_state(),
    }
