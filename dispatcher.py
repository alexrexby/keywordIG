...
# Чужой аккаунт — это почти всегда не чужой аккаунт, а перепутанный идентификатор, то есть
# ошибка настройки: она не рассосётся и повторяется на каждом событии. Сигналим с ПЕРВОГО
# и редко — здесь важно не пропустить начало, а не считать штуки.
FOREIGN_ALERT_COOLDOWN_SEC = 6 * 60 * 60
...
_budget_used = 0
# Последний entry_id, пришедший не от нашего аккаунта. Живёт в памяти, а не в таблице,
# намеренно: это СОСТОЯНИЕ настройки, а не история — после рестарта оно должно заполниться
# заново первым же событием, иначе панель будет показывать давно исправленную аварию.
last_foreign_entry_id: str | None = None
...
async def tick() -> bool:
    ...
    # ПОСЛЕ intake_once, а не внутри: там открыта транзакция, и HTTP-запрос под ней держал
    # бы соединение пула на всё время похода в Telegram.
    await note_foreign_account()
        ...
        async with db.pool_ref().connection() as conn:
            async with conn.transaction():
                ...
                if cand is None:
                    ...
                    return True
                foreign = foreign_account(event["payload"])
                if foreign:
                    ...
                    #
                    # Отметку кладём в глобальную переменную, а алерт шлём выше по стеку:
                    # эта ветка выполняется внутри транзакции.
                    global last_foreign_entry_id
                    last_foreign_entry_id = foreign
                    log.warning(
...
def foreign_account(payload: dict) -> str:
    """Чужой entry.id события или пустая строка. entry.id вебхука — это ПОЛУЧАТЕЛЬ.
    ...
    Возвращаем сам идентификатор, а не «да/нет»: в аварийном сообщении человеку нужно
    ЧИСЛО, которое он сравнит со своим, — без него совет «проверьте IG_USER_ID» ничем
    не помогает.
    ...
    return entry_id if entry_id and entry_id != IG_USER_ID else ""
...
async def release(d: dict) -> None:
    ...
    if d["dm_attempts"]:
        ...
        await alert(
            ...
            f"и снимите бронь у доставки {d['id']}: тогда на следующий комментарий "
            "материал уйдёт ему заново.\n"
...
async def note_foreign_account() -> None:
    """Событие пришло не нашему аккаунту. Сигналим с ПЕРВОГО, а не по накоплению.

    Молчаливее этой аварии нет ничего: вебхуки идут, подпись сходится, ошибок ноль,
    health зелёный, суточный расход нулевой — и не отвечает никто. Ни один из уже
    существующих сигналов её не ловит: очередь пуста, отказов нет, пауз нет.

    Текст называет ОБА идентификатора и говорит, какой из них нужен: у аккаунта их два,
    они разные, и перепутать их при заполнении окружения проще всего.
    """
    entry_id = last_foreign_entry_id
    if not entry_id:
        return
    await alert(
        "чужой аккаунт",
        "Instagram: комментарии приходят, но сервис не отвечает ни на один — они "
        "адресованы аккаунту, которого он не обслуживает.\n"
        "Почти всегда причина одна: в IG_USER_ID записан не тот идентификатор. У аккаунта "
        "их два, и они разные. Нужен тот, который Instagram сам присылает в уведомлении "
        f"(поле entry.id) — это {entry_id}. Тот, что возвращает запрос /me, привязан к "
        f"приложению, в уведомлениях не встречается и сюда не годится — а ждёт сервис "
        f"сейчас именно {IG_USER_ID}.\n"
        f"Что сделать: если {entry_id} — ваш аккаунт, впишите это число в IG_USER_ID и "
        "поднимите сервис заново. Если аккаунт действительно чужой, значит одно "
        "приложение Meta подписано на два аккаунта — так нельзя, второй молча "
        f"перехватывает события первого.\n"
        f"Сколько событий уже пропущено по этой причине, видно здесь: {PANEL_URL}.",
        FOREIGN_ALERT_COOLDOWN_SEC,
    )
